...
const templates = {
  badges: (folder, activeFilter) => {
    ...
    if (activeFilter === "all") {
      return `<div class="organizer-folder-badge">${Icons.getSvg(
        "image"
      )} ${ssCount}</div><div class="organizer-folder-badge">${Icons.getSvg(
        "video"
      )} ${srCount}</div>`;
    }
...
const render = {
  ...
  mediaCounter: (folders, activeFilter) => {
    ...
    if (activeFilter === "all") {
      ...
      elements.counter.innerHTML = `<span>${Icons.getSvg("folder")} ${
        filtered.length
      }</span><span>${Icons.getSvg("file")} ${totalSs + totalSr}</span>`;
    } else {
      ...
      elements.counter.innerHTML = `<span>${Icons.getSvg("folder")} ${
        filtered.length
      }</span><span>${Icons.getSvg(isRec ? "video" : "image")} ${total}</span>`;
    }
  },
