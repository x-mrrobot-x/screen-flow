const Icons = (function () {
  ...
  const DATA = {
    ...
    "hard-drive":
      '<path d="M22 12H2"/><path d="M5.45 5.11 2 12v6a2 2 0 0 0 2 2h16a2 2 0 0 0 2-2v-6l-3.45-6.89A2 2 0 0 0 16.76 4H7.24a2 2 0 0 0-1.79 1.11z"/><line x1="6" x1="6.01" y1="16" y2="16"/><line x1="10" x1="10.01" y1="16" y2="16"/>',
    clock:
    ...
    "folder-sync":
      '<path d="M9 20H4a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h3.9a2 2 0 0 1 1.69.9l.81 1.2a2 2 0 0 0 1.67.9H20a2 2 0 0 1 2 2v1"/><path d="M12 10v4h4"/><path d="m12 14 1.5-1.5c.9-.9 2.2-1.5 3.5-1.5s2.6.6 3.5 1.5l1.5 1.5"/><path d="M22 22v-4h-4"/><path d="m22 18-1.5 1.5c-.9.9-2.1 1.5-3.5 1.5s-2.6-.6-3.5-1.5L12 18"/>',
  ...
  function getIcon(name, attrs = {}) {
    ...
    const mergedAttrs = { ...DEFAULT_ATTRS, ...attrs };
    return buildSvg(content, mergedAttrs);
  }

  function getFolderIcon(item, attrs = {}) {
    const className = attrs.class ? ` ${attrs.class}` : "";
    const workDir = ENV.getWorkDir()

    return `<img src="src/assets/icons/${item.pkg}.png" alt="${item.name}" class="folder-icon${className}" onerror="this.onerror=null; this.src='${workDir}src/assets/icons/default.png';"
    />`;
  }

  function init() {
    const elements = DOM.qsa("[data-icon]");
    elements.forEach(el => {
