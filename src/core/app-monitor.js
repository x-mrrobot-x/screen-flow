const AppMonitor = (() => {
  ...
  function updateFolderInState(oldName, newName, ssResult, srResult) {
    ...
    const folder = { ...folders[idx], name: newName };

    if (ssResult.timestamp && folder.ss)
      folder.ss = { ...folder.ss, mtime: ssResult.timestamp };
    if (srResult.timestamp && folder.sr)
      folder.sr = { ...folder.sr, mtime: srResult.timestamp };
  ...
  async function fetchMediaFolders() {
    ...
    return {
      screenshotFolders:
        screenshotFolders.status === "fulfilled"
          ? screenshotFolders.value || []
          : [],
      recordingFolders:
        recordingFolders.status === "fulfilled"
  ...
  async function updateAppsData(newApps) {
    ...
    try {
      ...
      const appsToAdd = newApps.filter(app => !existingPkgs.has(app.pkg));
  ...
  async function processNewPackages(allPackages, existingApps) {
    const existingPkgs = createPackageSet(existingApps);
    const newPackages = allPackages.filter(pkg => !existingPkgs.has(pkg));
